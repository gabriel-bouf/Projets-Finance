...
def index_SP_avec_risque(ticker:str,periode:str,risk:int,fenetre:int,lag:int,EWA:int,plot:bool):
    ...
    returns.insert(2,"Daily Volatility",volatility)
    #returns.insert(3,"Annualized returns",(1+returns["Returns"])**252-1)
    returns.insert(3,"Annualized Volatility",returns["Daily Volatility"]*sqrt(252))
    returns.insert(4,"Controled returns",returns["Close"])

    returns["Controled returns"]=returns.apply(lambda row: row["Returns"]*((risk*0.01)/row["Annualized Volatility"]) if row["Annualized Volatility"]>(risk*0.01) else row["Returns"],axis=1)
